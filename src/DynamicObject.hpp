...
public:
    // DynamicObject operator=(const DynamicObject &_right) {
    //     DynamicObject res;

    //     res.N = N;
    //     res.m_positions = std::vector<glm::dvec3>(m_positions);
    //     res.m_velocities = std::vector<glm::dvec3>(m_velocities);
    //     res.m_masses = std::vector<double>(m_masses);
    //     res.m_weights = std::vector<double>(m_weights);
    //     res.m_fixed = std::vector<bool>(m_fixed);

    //     res.M = M;
    //     res.Mcoll = Mcoll;
    //     res.m_cardinalities = std::vector<uint>(m_cardinalities);
    //     res.m_functions = std::vector<constraint_function>(m_functions);
    //     res.m_gradients = std::vector<gradient_function>(m_gradients);
    //     res.m_indices = std::vector<std::vector<uint>>(m_indices);
    //     res.m_stiffnesses = std::vector<double>(m_stiffnesses);
    //     res.m_types = std::vector<ConstraintType>(m_types);

    //     res.m_ambient_friction_coefficient = m_ambient_friction_coefficient;
    //     res.m_friction_coefficient = m_friction_coefficient;
    //     res.m_restitution_coefficient = m_restitution_coefficient;
